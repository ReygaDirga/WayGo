<div class="relative w-[calc(100%-2rem)] md:w-[calc(100%-4rem)] max-w-[1100px] mx-auto h-[500px] md:h-[550px] rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl mt-10">
    
    <div id="carousel-slides" class="w-full h-full relative">
        
        @foreach($heroPosts as $index => $hp)
        <div class="carousel-item cursor-pointer absolute inset-0 transition-opacity duration-700 ease-in-out {{ $loop->first ? 'opacity-100 z-10' : 'opacity-0 z-0' }}">
            
            <img src="{{ $hp->image ? asset('storage/' . $hp->image) : asset('assets/Logo1.png') }}" class="w-full h-full object-cover" alt="{{ $hp->title }}">
            
            <div class="absolute inset-0 bg-gradient-to-t from-black/95 via-black/50 to-transparent"></div>
            
            <div class="absolute inset-0 text-white">
                <div class="absolute top-6 left-6 md:top-10 md:left-10">
                    <span class="bg-[#1A365D] px-4 py-2 md:px-5 md:py-2.5 rounded-full text-[10px] md:text-xs font-semibold flex items-center w-max gap-2 md:gap-2.5">
                        ⭐ Featured Story
                    </span>
                </div>

                <div class="absolute bottom-10 left-6 right-6 md:bottom-12 md:left-10 md:right-10">
                    <h2 class="text-3xl md:text-5xl font-bold leading-tight mb-3 md:mb-4">{{ $hp->title }}</h2>
                    
                    <div class="flex flex-wrap items-center gap-2 md:gap-3 text-xs md:text-sm text-gray-300 mb-5 md:mb-6">
                        <span class="flex items-center gap-1 md:gap-1.5 whitespace-nowrap">
                            <svg class="w-3.5 h-3.5 md:w-4 md:h-4 text-white" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 21C16.9706 17.3824 20 13.3824 20 9C20 4.58172 16.4183 1 12 1C7.58172 1 4 4.58172 4 9C4 13.3824 7.02944 17.3824 12 21Z" stroke="currentColor" stroke-width="2"/><circle cx="12" cy="9" r="3" stroke="currentColor" stroke-width="2"/></svg>
                            {{ explode(',', $hp->location)[0] }}, {{ $hp->pulau->name ?? '' }}
                        </span>
                        <span class="text-gray-500 hidden md:inline">|</span>
                        <span class="flex items-center gap-1 md:gap-1.5 whitespace-nowrap">
                            <svg class="w-3.5 h-3.5 md:w-4 md:h-4 text-white" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M19 4H5C3.89543 4 3 4.89543 3 6V18C3 19.1046 3.89543 20 5 20H19C20.1046 20 21 19.1046 21 18V6C21 4.89543 20.1046 4 19 4Z" stroke="currentColor" stroke-width="2"/><path d="M16 2V6M8 2V6M3 10H21" stroke="currentColor" stroke-width="2"/><path d="M11 13H13V15H11V13Z" stroke="currentColor" stroke-width="2"/></svg>
                            {{ $hp->created_at->format('F d, Y') }} by {{ $hp->user->name ?? 'Traveler' }}
                        </span>
                    </div>

                    <a href="{{ route('blog-detail',$hp->id) }}" class="inline-block bg-[#1A365D] hover:bg-blue-800 transition px-5 py-2 md:px-6 md:py-2.5 rounded-full text-xs md:text-sm font-semibold">
                        Read Full Story
                    </a>
                </div>
            </div>
        </div>
        @endforeach

    </div>

    <div class="absolute bottom-4 md:bottom-6 left-1/2 transform -translate-x-1/2 flex space-x-2 md:space-x-3 z-20">
        @foreach($heroPosts as $index => $post)
        <button class="carousel-dot w-2 h-2 md:w-3 md:h-3 rounded-full transition {{ $loop->first ? 'bg-orange-400' : 'bg-white' }}" onclick="goToSlide({{ $index }})"></button>
        @endforeach
    </div>
</div>